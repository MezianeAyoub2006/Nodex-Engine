...
class Window:
    ...
    def window_scale(self):
        if self.fullscreen:
            return self.fullscreen_viewport()[2] / self._internal_size[0]
        return self._window_scale
    
    def in_display(self, position):
        EXTENSION = 20
        screen_rect = pygame.Rect(
            - 2.5 * EXTENSION, 
            0, 
            self.internal_size[0] + 5 * EXTENSION, 
            self.internal_size[1] + 20 * EXTENSION
        )
        return screen_rect.collidepoint(position)